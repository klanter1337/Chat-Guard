...
async function Punish(message, GuardType, Embed) {
    let result = false;
    let MuteSüresi = config.MuteDurationMinute;

    if(message.member.roles.cache.has(config.MutedRoleID)) return;
    if(config.MuteDurationMinute !== Number)MuteSüresi = 60;
    ...
    if(result === true) {
        ...
        setTimeout(async() => { 
            ...
            message.member.roles.remove(MuteRole); }, MuteSüresi * 1000 * 60 ); 
        ...
        if(GuardType === 'MassPingGuard') {
            ...
            return client.channels.cache.get(config.PunishLogChannelID).send(Embed.setDescription('<@'+message.author.id+'>, Nickli kullanıcı mesajında **'+message.mentions.users.size+'** kişiyi etiketlediği için **'+MuteSüresi+'** Dakika boyunca mutelendi.'));
        }
        if(GuardType === 'InviteGuard') {
            message.channel.send(Embed.setDescription('<@'+message.author.id+'>, Sunucu daveti attığın için **'+MuteSüresi+'** Dakika Mute yedin. Lütfen kurallara uymaya özen göster'));
            return client.channels.cache.get(config.PunishLogChannelID).send(Embed.setDescription('<@'+message.author.id+'>, Nickli kullanıcı **'+message.content+'** içerikli mesajında sunucu daveti bulunduğu için **'+MuteSüresi+'** Dakika boyunca mutelendi.'));
        }
        if(GuardType === 'SpamGuard') {
            message.channel.send(Embed.setDescription('<@'+message.author.id+'>, Spam yaptığın için **'+MuteSüresi+'** Dakika Mute yedin. Lütfen kurallara uymaya özen göster'));
            return client.channels.cache.get(config.PunishLogChannelID).send(Embed.setDescription('<@'+message.author.id+'>, Nickli kullanıcı Spam yaptığı için **'+MuteSüresi+'** Dakika boyunca mutelendi.'));
        }
    }
    if(result === false) {
        ...
        if(GuardType === 'MassPingGuard') {
            ...
            return client.channels.cache.get(config.PunishLogChannelID).send(Embed.setDescription('<@'+message.author.id+'>, Nickli kullanıcı mesajında **'+message.mentions.users.size+'** kişiyi etiketlediği için uyarı aldı.'));
        }
        if(GuardType === 'InviteGuard') {
            message.channel.send(Embed.setDescription('<@'+message.author.id+'>, Sunucu Daveti atman yasak, tekrarı durumunda ceza alacaksınız !'));
            return client.channels.cache.get(config.PunishLogChannelID).send(Embed.setDescription('<@'+message.author.id+'>, Nickli kullanıcı **'+message.content+'** içerikli mesajında sunucu daveti bulunduğu için uyarı aldı.'));
        }
        if(GuardType === 'SpamGuard') {
            message.channel.send(Embed.setDescription('<@'+message.author.id+'>, Spam yapman yasak, tekrarı durumunda ceza alacaksınız !'));
            return client.channels.cache.get(config.PunishLogChannelID).send(Embed.setDescription('<@'+message.author.id+'>, Nickli kullanıcı Spam yaptığı için uyarı aldı.'));
        }
    }
    return result;
}
